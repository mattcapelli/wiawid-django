...
class SendUserMail:
    ...
    def __sign_up_compose_mail(self):
        ...
        Thanks so much for signing up for WiaWid!\n
        Use the link below to confirm your account:\n
        {self.link}\n
        If you didn't sign up for WiaWid, please ignore this email.\n
        Sincerely,\n
        WiaWid Support
        """
        logger.info(content)
    ...
    def __sign_up_alert_compose_mail(self):
        content = f"""
        Hello {self.recipient_name.title()},\n
        A new user registered for WiaWid!\n
        Sincerely,\n
        WiaWid Support
        """
        logger.info(content)
    ...
    def __password_change_compose_mail(self):
        content = f"""
        Hello {self.recipient_name.title()},\n
        Your WiaWid password for the account with the email {self.recipient_list} has changed. \n
    ...
    def send(self):
        try:
            send_mail(subject=self.subject, message=self.content, from_email='WiaWid <' + self.from_email + '>', recipient_list=[self.recipient_list], fail_silently=True)
            return True
        except Exception:
            return False
